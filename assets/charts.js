(function () {
    ...
    function renderBarChart(canvasId, labels, datasets, showMoney, tooltipOptions) {
        ...
        new Chart(canvas, {
            ...
            options: {
                ...
                maintainAspectRatio: false,
                plugins: {
                    legend: { position: datasets.length > 1 ? 'bottom' : false },
                    tooltip: tooltipOptions || (showMoney ? moneyTooltip() : undefined)
                },
                scales: {
                    y: {
                        beginAtZero: true,
                        ticks: showMoney ? moneyTicks() : undefined
                    }
                }
            }
        });
    ...
    function renderDashboardCharts(data) {
        ...
        const cashTooltip = moneyTooltip({
            ...
            afterBody: function (items) {
                ...
                return [
                    ...
                    'Cash Out: ' + money(valueAt(data.cashOut, index, 0)),
        ...
        const projectTooltip = moneyTooltip({
            title: function (items) {
                return 'Project: ' + (items[0] ? items[0].label : '');
            },
            label: function (context) {
                return context.dataset.label + ': ' + money(context.parsed.y);
            },
            afterBody: function (items) {
                const index = items[0] ? items[0].dataIndex : 0;
                return [
                    'Income: ' + money(valueAt(data.projectIncome, index, 0)),
                    'Expense: ' + money(valueAt(data.projectExpense, index, 0)),
                    'Net Income: ' + money(valueAt(data.projectNet, index, 0))
                ];
            }
        });

        renderBarChart('dashboardProjectChart', data.projectLabels || [], [
            {
                label: 'Income',
                data: data.projectIncome || [],
                backgroundColor: 'rgba(20, 83, 45, 0.78)',
                borderColor: '#14532D',
                borderWidth: 1
            },
